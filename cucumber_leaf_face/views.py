...
def showall(request):
    ...
    return render(request, "cucumber_leaf_face/showall.html", context)
...
def upload(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            img.save()
            return redirect("cucumber_leaf_face:result")
    else:
        form = ImageForm()
    context = {"form": form}
    return render(request, "cucumber_leaf_face/upload.html", context)

def result(request):
    images = Image.objects.all().order_by("-pk")
    context = {"images": images[1:], "now_image": images[0]}
    return render(request, "cucumber_leaf_face/result.html", context)
